...
function withValidator(callback) {
  ...
  if (!fs.existsSync(JAVA_HOME + '/bin/javac') && !fs.existsSync(JAVA_HOME + '/bin/javac.exe')) {
...
function validateXML(xml, schema, callback) {
  ...
  withValidator(function(err) {
    ...
    var validator = spawn(JAVA_HOME + '/bin/java', [ '-cp', __dirname + '/../', 'support.XMLValidator', input, '-schema=' + schema ]);
